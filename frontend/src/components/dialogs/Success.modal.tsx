...
import type { ModalState } from "../dashboard_layout/AddAccount"; // adjust path if needed

interface SuccessBelvoModalProps {
  setModal: React.Dispatch<React.SetStateAction<ModalState>>;
}


export default function SuccessBelvoModal({ setModal }: SuccessBelvoModalProps) {
	const cancelModal = () => {
		setModal((prev) => ({ ...prev, isSucccess: false }));
    ...
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-lg relative animate-fade-in">
          <div className="w-[48px] h-[48px] sm:w-[58px] sm:h-[58px] content-center mx-auto">
            {/* Replace this SVG with your BelvoSuccess component if you want */}
          ...
          <h2 className="font-semibold mb-2 mt-5 text-center text-[25px]">Su cuenta se vinculó con éxito</h2>
          <p className="text-sm text-gray-600 text-center mt-5">Es hora de administrar tus suscripciones y manejar mejor tu dinero. </p>
